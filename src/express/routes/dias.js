...
async function getAll(req, res) {
	const entities = await models.dia.findAll();  // Cambia 'dia' por la entidad deseada.
...
async function getById(req, res) {
	...
	const entity = await models.dia.findByPk(id);  // Cambia 'dia' por la entidad deseada.
	if (entity) {
...
async function create(req, res) {
	if (req.body.id) {
		...
		await models.dia.create(req.body);  // Cambia 'dia' por la entidad deseada.
...
async function update(req, res) {
	...
	if (req.body.id === id) {
		await models.dia.update(req.body, {  // Cambia 'dia' por la entidad deseada.
			where: {
...
async function remove(req, res) {
	const id = getIdParam(req);  // Valida y obtiene el ID del parámetro de la URL.
	await models.dia.destroy({  // Cambia 'dia' por la entidad deseada.
		where: {
