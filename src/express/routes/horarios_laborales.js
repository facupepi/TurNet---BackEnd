...
async function getHorariosByServicio(req, res) {
	...
	try {
	  const horariosLaborales = await models.horarios_laborales.findAll({
		where: {
	  ...
	  res.status(200).json(horariosLaborales);  // Devuelve los horarios encontrados con los campos 'hora'.
	} catch (error) {
...
async function create(req, res) {
    ...
    try {
        ...
        while (currentHora <= horaFinDate) {
            ...
            const horaFormateada = currentHora.toTimeString().slice(0, 8); // Aseguramos formato HH:MM:SS
            ...
            // Si el horario existe en la base de datos, lo agregamos a la lista de horarios laborales
            if (horario) {
                horariosLaborales.push({
                    id_servicio: id_servicio,
                    id_horario: horario.id, // Usamos el ID del horario recuperado
        ...
        // Usamos bulkCreate para insertar todos los registros en la tabla 'horarios_laborales'
        await models.horarios_laborales.bulkCreate(horariosLaborales.map(h => ({
        ...
        return res.status(201).json({
            ...
            horarios: horariosLaborales.map(h => ({
                id_horario: h.id_horario,
                hora: h.hora
            })) // Incluimos tanto el ID como la hora
        });
